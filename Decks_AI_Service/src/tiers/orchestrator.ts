...
const OrchestratorResultSchema = z.object({
  reasoningTrace: z
    .string()
    .describe('A short first-person reasoning trace (2-4 numbered points) explaining how the brief was parsed and why a clarifying question is needed.'),
  clarifyQuestion: z.string(),
  clarifyOptions: z.array(z.string()).min(2).max(6),
...
const JSON_SCHEMA = {
  ...
  properties: {
    ...
    clarifyOptions: { type: 'array', items: { type: 'string' }, minItems: 2, maxItems: 6 },
    copyDirective: {
      type: 'object',
      properties: {
  ...
  required: ['reasoningTrace', 'clarifyQuestion', 'clarifyOptions', 'copyDirective', 'designDirective'],
  additionalProperties: false,
} as const
...
function fallbackResult(): OrchestratorResult {
  return {
    reasoningTrace:
      ...
      '2. The goal is not fully specified in the prompt\n' +
      "3. I'll ask one quick clarifying question to lock the goal before drafting sections",
    clarifyQuestion: "What's the primary goal for this deck?",
    clarifyOptions: ['Pitch to investors', 'Internal team update', 'Customer-facing overview', 'Conference talk'],
...
2. Ask exactly ONE clarifying question about the deck's primary GOAL (never ask about the user's identity), with 2-6 concrete options.
